...
/**
 * CustomSlider Sliders Class
 *
 * All functionality pertaining to the slideshows in CustomSlider.
 *
 * @package WordPress
...
class CustomSlider_Sliders {
	...
	public function get_slides ( $type, $args = array() ) {
		...
		$supported_types = CustomSlider_Utils::get_slider_types();
			...
			foreach ( $attachments as $k => $v ) {
				...
				if ( $args['thumbnails'] == 'true' ) {
					...
					if ( ! is_bool( $thumb_url ) ) {
						...
						$data['attributes'] = array( 'data-thumb' => esc_url( CustomSlider_Utils::get_placeholder_image() ) );
					}
				}
				$slides[] = $data;
			}
	...
	private function slideshow_type_posts ( $args = array() ) {
		...
		$supported_layouts = CustomSlider_Utils::get_posts_layout_types();
		...
		if ( ! is_wp_error( $posts ) && ( count( $posts ) > 0 ) ) {
			...
			foreach ( $posts as $k => $post ) {
				...
				$excerpt = '';
				if ( ( $args['display_excerpt'] == 'true' || $args['display_excerpt'] == 1 ) && has_excerpt( get_the_ID() ) ) { $excerpt = wpautop( get_the_excerpt( get_the_ID() ) ); }
				...
				$layed_out_content = apply_filters( 'laboratory_slideshow_posts_layout_html', $content, $args, $post );

				$content = '<div class="' . esc_attr( $class ) . '">' . $layed_out_content . '</div>';
				$data = array( 'content' => $content );

				if ( $args['thumbnails'] == 'true' || $args['thumbnails'] == 1 ) {
	...
	public function apply_default_filters_slides () {
		add_filter( 'laboratory_slideshow_slide_content_slides', 'do_shortcode' );
		add_filter( 'laboratory_slideshow_slide_content_slides', 'wptexturize' );
