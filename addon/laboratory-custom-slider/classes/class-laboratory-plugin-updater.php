<?php
/**
 * ColorLabs Plugin Update Checker
 *
 * Use the WordPress Update Manager to check for plugin updates at ColorLabs.com.
 *
 * @version 1.0.0
 * @category Plugins
 * @package WordPress
 ...
 * - hide_admin_notice()
 */
	class Laboratory_Plugin_Updater {
		var $api_url = 'https://colorlabsproject.com/laboratory-api';
		var $api_key = ''; // Unique value, used to determine the plugin to be updated.
		
		var $plugin_token = 'laboratory-plugin-updater';
		var $plugin_path;
	 	var $plugin_url;
	 	var $plugin_prefix = 'laboratory_plugin_updater_';
	 	var $plugin_base;
		...
		/**
		 * Laboratory_Plugin_Updater function.
		 * 
		 * @access public
		 * @return void
		 */
		function Laboratory_Plugin_Updater ( $file ) {
		...
		function register_nav_menu_link () {	
			...
			if ( function_exists( 'add_submenu_page' ) ) {
				$this->admin_screen = add_submenu_page( 'index.php', __( 'ColorLabs Updates', 'laboratory' ), __( 'ColorLabs Updates', 'laboratory' ), 'switch_themes', $this->plugin_token, array( &$this, 'admin_screen' ) );	
			}
			
			// Load admin screen logic.
			if ( isset( $_POST['laboratory-action'] ) && ( $_POST['laboratory-action'] == 'laboratory-plugin-updater-login' ) ) {
...
<div class="wrap">
	...
	<h2><?php _e( 'ColorLabs Plugin Updater', 'laboratory' ); ?></h2>
	
	<form name="laboratory-plugin-updater-login" id="laboratory-plugin-updater-login" action="<?php echo admin_url( 'index.php?page=' . $this->plugin_token ); ?>" method="post">
		<fieldset>
			<table class="form-table">
				<tbody>
					<tr>
						<th scope="row"><label for="username"><?php _e( 'ColorLabs Username', 'laboratory' ); ?>:</label></th>
						<td><input type="text" class="input-text input-laboratory_user regular-text" name="username" id="laboratory_user" value="" /></td>
					</tr>
					<tr>
						<th scope="row"><label for="password"><?php _e( 'ColorLabs Password', 'laboratory' ); ?>:</label></th>
						<td><input type="password" class="input-text input-laboratory_pass regular-text" name="password" id="laboratory_pass" value="" /></td>
					</tr>
				</tbody>
			</table>
		</fieldset>
		...
		<fieldset>
			<p class="submit">
				<button type="submit" name="laboratory_login" id="laboratory_login" class="button-primary"><?php _e( 'Login', 'laboratory' ); ?></button>
			</p>
			<input type="hidden" name="laboratory-action" value="laboratory-plugin-updater-login" />
			<input type="hidden" name="page" value="laboratory-plugin-updater" />
		</fieldset>
	</form>
...
<?php
		...
		function admin_notice () {
			...
			if ( isset( $_GET['page'] ) && ( $_GET['page'] == $this->plugin_token ) ) {
				...
				if ( $this->username != '' && $this->password != '' ) {	
					$notice = '<div id="laboratory-plugin-updater-notice" class="updated fade">' . "\n";
					$notice .= '<p><strong>' . __( 'Switch ColorLabs Account', 'laboratory' ) . '</strong></p><p>' . "\n";
					
					$notice .= sprintf( __( 'You are currently logged in as %1$s. To switch to a different ColorLabs account, please fill in the login details below.', 'laboratory' ), '<strong>' . $this->username . '</strong>' );
					$notice .= "\n" . '</p>' . "\n";
					
					$notice .= '<p><a href="' . admin_url( 'update-core.php' ) . '">' . __( 'Update your Plugins', 'laboratory' ) . ' &rarr;</a></p>' . "\n";
					
					$notice .= '</div>' . "\n";
				}
				
				if ( isset( $_GET['type'] ) && ( $_GET['type'] == 'error' ) ) {	
					$notice .= '<div id="laboratory-plugin-updater-error-notice" class="error fade">' . "\n";
					$notice .= '<p><strong>' . __( 'Login Error', 'laboratory' ) . '</strong></p><p>' . "\n";
					
					$notice .= __( 'The login details supplied are invalid. Please try again.', 'laboratory' );
					$notice .= "\n" . '</p>' . "\n";
			...
			if ( ( ! isset( $_GET['page'] ) || ( isset( $_GET['page'] ) && ( $_GET['page'] != $this->plugin_token ) ) ) && ( $this->username == '' || $this->password == '' ) && ( get_option( $this->plugin_prefix . 'hide-admin-notice', false ) != true ) ) {
				$notice = '<div id="laboratory-plugin-updater-notice" class="updated fade">' . "\n";
				$notice .= '<p class="alignleft"><strong>' . __( 'Enable ColorLabs Plugin Updates.', 'laboratory' ) . '</strong> ' . "\n";
				
				$notice .= sprintf( __( 'Please <a href="%1$s">login</a> to enable automatic plugin updates.', 'laboratory' ), 'index.php?page=' . $this->plugin_token );
				$notice .= "\n" . '</p>' . "\n";
				...
				$url .= $delimiter . 'laboratory-hide-updatenotice=true';
				
				$notice .= '<p class="alignright submitbox"><a href="' . $url . '" class="submitdelete">' . __( 'Hide This Message', 'laboratory' ) . '</a></p>' . "\n";
		...
		function hide_admin_notice () {
			if ( isset( $_GET['laboratory-hide-updatenotice'] ) && ( $_GET['laboratory-hide-updatenotice'] == 'true' ) ) {
				update_option( $this->plugin_prefix . 'hide-admin-notice', true );
				
				$url = 'http://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
				
				$url = str_replace( 'laboratory-hide-updatenotice=true', '', $url );
