...
/**
 * CustomSlider Post Types Class
 *
 * All functionality pertaining to the post types and taxonomies in CustomSlider.
 *
 * @package WordPress
 ...
 * - remove_laboratoryframework_metabox()
 * - enter_title_here()
 */
class CustomSlider_PostTypes {
	public $token;
	 ...
	 * @since  1.0.0
	 * @uses  global $laboratory_slideshow
	 * @return void
	 */
	...
	public function setup_slide_pages_taxonomy () {
		...
		$labels = array(
			'name' => _x( 'Slide Groups', 'taxonomy general name', 'laboratory' ),
			'singular_name' => _x( 'Slide Group', 'taxonomy singular name', 'laboratory' ),
			'search_items' =>  __( 'Search Slide Groups', 'laboratory' ),
			'all_items' => __( 'All Slide Groups', 'laboratory' ),
			'parent_item' => __( 'Parent Slide Group', 'laboratory' ),
			'parent_item_colon' => __( 'Parent Slide Group:', 'laboratory' ),
			'edit_item' => __( 'Edit Slide Group', 'laboratory' ), 
			'update_item' => __( 'Update Slide Group', 'laboratory' ),
			'add_new_item' => __( 'Add New Slide Group', 'laboratory' ),
			'new_item_name' => __( 'New Slide Group Name', 'laboratory' ),
			'menu_name' => __( 'Slide Groups', 'laboratory' )
		); 	
	...
	private function setup_post_type_labels_base () {
		...
		$this->labels['slide'] = array( 'singular' => __( 'Slide', 'laboratory' ), 'plural' => __( 'Slides', 'laboratory' ), 'menu' => __( 'Slideshows', 'laboratory' ) );
	...
	private function create_post_type_labels ( $token, $singular, $plural, $menu ) {
		$labels = array(
		    'name' => sprintf( _x( '%s', 'post type general name', 'laboratory' ), $plural ),
		    'singular_name' => sprintf( _x( '%s', 'post type singular name', 'laboratory' ), $singular ),
		    'add_new' => sprintf( _x( 'Add New %s', $token, 'laboratory' ), $singular ),
		    'add_new_item' => sprintf( __( 'Add New %s', 'laboratory' ), $singular ),
		    'edit_item' => sprintf( __( 'Edit %s', 'laboratory' ), $singular ),
		    'new_item' => sprintf( __( 'New %s', 'laboratory' ), $singular ),
		    'all_items' => sprintf( __( 'All %s', 'laboratory' ), $plural ),
		    'view_item' => sprintf( __( 'View %s', 'laboratory' ), $singular ),
		    'search_items' => sprintf( __( 'Search %s', 'laboratory' ), $plural ),
		    'not_found' =>  sprintf( __( 'No %s found', 'laboratory' ), strtolower( $plural ) ),
		    'not_found_in_trash' => sprintf( __( 'No %s found in Trash', 'laboratory' ), strtolower( $plural ) ), 
		    'parent_item_colon' => '',
		    'menu_name' => $menu
		  );
	...
	private function create_post_type_messages ( $post_type ) {
		...
		$messages = array(
			...
			2 => __( 'Custom field updated.', 'laboratory' ),
			3 => __( 'Custom field deleted.', 'laboratory' ),
			4 => sprintf( __( '%s updated.', 'laboratory' ), esc_attr( $this->labels[$post_type]['singular'] ) ),
			/* translators: %s: date and time of the revision */
			5 => isset( $_GET['revision']) ? sprintf( __('%2$s restored to revision from %1$s', 'laboratory' ), wp_post_revision_title( (int) $_GET['revision'], false ), esc_attr( $this->labels[$post_type]['singular'] ) ) : false,
			6 => sprintf( __('%2$s published.' ), esc_url( get_permalink($post_ID) ), esc_attr( $this->labels[$post_type]['singular'] ) ),
			7 => sprintf( __( '%s saved.', 'laboratory' ),  esc_attr( $this->labels[$post_type]['singular'] ) ),
			8 => sprintf( __( '%2$s submitted.', 'laboratory' ), esc_url( add_query_arg( 'preview', 'true', get_permalink( $post_ID ) ) ), esc_attr( $this->labels[$post_type]['singular'] ) ),
			9 => sprintf( __( '%s scheduled for: <strong>%1$s</strong>.', 'laboratory' ),
			...
			10 => sprintf( __( '%s draft updated.', 'laboratory' ), esc_attr( $this->labels[$post_type]['singular'] ) ),
		);
	...
	/**
	 * Remove the LaboratoryFramework meta box from the "Sliders" admin screens.
	 * @since  1.0.0
	 * @return void
	 */
	public function remove_laboratoryframework_metabox () {
		remove_meta_box( 'laboratory-settings', 'slide', 'normal' );
	} // End remove_laboratoryframework_metabox()
	...
	public function enter_title_here ( $title ) {
		if ( get_post_type() == 'slide' ) {
			$title = __( 'Enter a title for this slide here', 'laboratory' );
		}
	...
	public function add_column_headings ( $defaults ) {
		...
		$new_columns['title'] = _x( 'Slide Title', 'column name', 'laboratory' );
		$new_columns['slide-thumbnail'] = _x( 'Featured Image', 'column name', 'laboratory' );
		$new_columns['slide-page'] = _x( 'Slide Groups', 'column name', 'laboratory' );
	...
	public function add_column_data ( $column_name, $id ) {
		...
		switch ( $column_name ) {
			...
			case 'slide-page':
				$value = __( 'No Slide Groups Specified', 'laboratory' );
