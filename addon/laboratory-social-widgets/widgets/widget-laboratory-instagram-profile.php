...
/**
 * Laboratory Instagram Profile Widget
 *
 * A bundled Laboratory Instagram profile widget.
 *
 * @package WordPress
 ...
 */
class Laboratory_Widget_InstagramProfile extends WP_Widget {
	...
	public function __construct () {
		...
		$this->laboratory_widget_cssclass = 'widget_laboratory_instagram_profile';
		$this->laboratory_widget_description = __( 'This is a Laboratory bundled Instagram profile widget.', 'laboratory' );
		$this->laboratory_widget_idbase = 'laboratory_instagram_profile';
		$this->laboratory_widget_title = __('Laboratory - Instagram Profile', 'laboratory' );
   ...
   public function form ( $instance ) {
		...
		$defaults = array(
						'title' => __( 'Instagram Profile', 'laboratory' ), 
						'access_token' => '', 
						'username' => '', 
						'password' => '', 
...
?>
	...
	<p>
		<label for="<?php echo $this->get_field_id( 'avatar_alignment' ); ?>"><?php _e( 'Avatar Alignment:', 'laboratory' ); ?></label>
		<select name="<?php echo $this->get_field_name( 'avatar_alignment' ); ?>" class="widefat" id="<?php echo $this->get_field_id( 'avatar_alignment' ); ?>">
			<option value="left"<?php selected( $instance['avatar_alignment'], 'left' ); ?>><?php _e( 'Left', 'laboratory' ); ?></option>
			<option value="centre"<?php selected( $instance['avatar_alignment'], 'centre' ); ?>><?php _e( 'Centre', 'laboratory' ); ?></option>
			<option value="right"<?php selected( $instance['avatar_alignment'], 'right' ); ?>><?php _e( 'Right', 'laboratory' ); ?></option>         
		</select>
	</p>
	<?php foreach ( $checkboxes as $k => $v ) { ?>
	<!-- Widget <?php echo $v; ?>: Checkbox Input -->
...
<?php
		...
		if ( isset( $data->data->id ) ) {
			if ( isset( $data->meta->code ) && $data->meta->code == 200 ) {
				...
				if ( $instance['display_media_count'] == 1 && isset( $data->data->counts->media ) ) {
					$html .= '<p class="media stat"><span class="number">' . $data->data->counts->media . '</span> <span class="stat-label">' . __( 'Media', 'laboratory' ) . '</span></p>' . "\n";
				}

				if ( $instance['display_followed_by_count'] == 1 && isset( $data->data->counts->followed_by ) ) {
					$html .= '<p class="followed_by stat"><span class="number">' . $data->data->counts->followed_by . '</span> <span class="stat-label">' . __( 'Followed By', 'laboratory' ) . '</span></p>' . "\n";
				}

				if ( $instance['display_follows_count'] == 1 && isset( $data->data->counts->follows ) ) {
					$html .= '<p class="follows stat"><span class="number">' . $data->data->counts->follows . '</span> <span class="stat-label">' . __( 'Follows', 'laboratory' ) . '</span></p>' . "\n";
				}
	...
	private function get_checkbox_settings () {
		return array(
					'display_avatar' => __( 'Display Avatar', 'laboratory' ), 
					'display_name' => __( 'Display Name', 'laboratory' ), 
					'display_screen_name' => __( 'Display Screen Name', 'laboratory' ), 
					'display_description' => __( 'Display Description', 'laboratory' ), 
					'display_website' => __( 'Display Website', 'laboratory' ), 
					'display_media_count' => __( 'Display Media Count', 'laboratory' ), 
					'display_followed_by_count' => __( 'Display Followed By Count', 'laboratory' ), 
					'display_follows_count' => __( 'Display Follows Count', 'laboratory' )
					);
