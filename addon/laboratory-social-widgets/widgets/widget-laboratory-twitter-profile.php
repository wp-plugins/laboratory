...
/**
 * Laboratory Twitter Profile Widget
 *
 * A bundled Laboratory Twitter Profile widget.
 *
 * @package WordPress
 ...
 * - enqueue_styles()
 */
class Laboratory_Widget_TwitterProfile extends WP_Widget {
	...
	function __construct () {
		...
		$this->laboratory_widget_cssclass = 'widget_laboratory_twitterprofile';
		$this->laboratory_widget_description = __( 'This is a Laboratory bundled Twitter profile widget.', 'laboratory' );
		$this->laboratory_widget_idbase = 'laboratory_twitterprofile';
		$this->laboratory_widget_title = __('Laboratory - Twitter Profile', 'laboratory' );
	...
	function widget( $args, $instance ) {
		...
		if ( $instance['display_friends_count'] == true && isset( $data->friends_count ) ) {
			$html .= '<p class="friends stat"><span class="number">' . $data->friends_count . '</span> <span class="stat-label">' . __( 'friends', 'laboratory' ) . '</span></p>' . "\n";
		}

		if ( $instance['display_follower_count'] == true && isset( $data->followers_count ) ) {
			$html .= '<p class="followers stat"><span class="number">' . $data->followers_count . '</span> <span class="stat-label">' . __( 'followers', 'laboratory' ) . '</span></p>' . "\n";
		}

		if ( $instance['display_status_count'] == true && isset( $data->statuses_count ) ) {
			$html .= '<p class="statuses stat"><span class="number">' . $data->statuses_count . '</span> <span class="stat-label">' . __( 'tweets', 'laboratory' ) . '</span></p>' . "\n";
		}
		...
		if ( $instance['display_tweeting_since'] == true && isset( $data->statuses_count ) ) {
			$html .= '<p class="tweeting-since">' . __( 'Tweeting since', 'laboratory' ) . ' <span class="date">' . date( get_option( 'date_format' ), strtotime( $data->created_at ) ) . '</span>' . '</p>' . "\n";
		}
   ...
   function form ( $instance ) {
		...
		$defaults = array(
						'title' => __( 'Twitter Profile', 'laboratory' ), 
						'twitter_handle' => '', 
						'display_avatar' => 1, 
						'display_name' => 1, 
...
<?php
	...
	private function get_checkbox_settings () {
		return array(
					...
					'display_location' => __( 'Display Location', 'laboratory' ), 
					'display_status_count' => __( 'Display Tweet Count', 'laboratory' ), 
					'display_follower_count' => __( 'Display Followers Count', 'laboratory' ), 
					'display_friends_count' => __( 'Display Friends Count', 'laboratory' ), 
					'display_tweeting_since' => __( 'Display "Tweeting Since"', 'laboratory' ), 
					'include_follow_link' => __( 'Include "Follow" Link', 'laboratory' )
					);
