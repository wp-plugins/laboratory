 ...
 * Laboratory - Megamenu
 *
 * Make your own list menu with this Mega Menu. Also you can add the favourite menu with the image and a little description within.
 *
 * @package WordPress
...
if( !class_exists( 'Laboratory_Megamenu' ) )
{	

	class Laboratory_Megamenu 
	{

		public $token = 'laboratory_megamenu';
		public $plugin_url;
		public $plugin_path;
		
		function Laboratory_Megamenu()
		{
			//adds stylesheet and javascript to the menu page
			add_action('admin_menu', array(&$this,'laboratory_menu_header'));
		
			//exchange arguments and tell menu to use the laboratory walker for front end rendering
			...
			//save laboratory options:
			add_action( 'wp_update_nav_menu_item', array(&$this,'update_menu'), 100, 3);
			
			add_action( 'wp_footer', array( &$this, 'frontend_enqueue_styles' ) );
		}
		
		function frontend_enqueue_styles () {

			wp_enqueue_style( 'laboratory-megamenu', esc_url( plugin_dir_url(plugin_basename(dirname(__FILE__))) . 'assets/laboratory-frontend-megamenu.css' ), '', '1.0.0', 'all' );

		} // End enqueue_styles()
		/**
		 * If we are on the nav menu page add javascript and css for the page
		 */
		function laboratory_menu_header()
		{
			if(basename( $_SERVER['PHP_SELF']) == "nav-menus.php" )
			{	
				global $laboratory_megamenu;
				wp_enqueue_style(  'laboratory_admin_style', esc_url( plugin_dir_url(plugin_basename(dirname(__FILE__))) . 'assets/laboratory-megamenu.css' )); 
				wp_enqueue_script( 'laboratory_mega_menu' , esc_url( plugin_dir_url(plugin_basename(dirname(__FILE__))) . 'assets/laboratory-megamenu.js' ),array('jquery'), false, true ); 
		...
		function modify_arguments($arguments){
			...
			$arguments['walker'] 						= new Laboratory_Walker();
			$arguments['container_class'] 	= $arguments['container_class'] .= ' Megamenu-Wrapper';
			$arguments['menu_class']				= 'laboratory_mega';
		...
		{
			return 'Laboratory_Backend_Walker';
		}
		...
		{	
			...
			{
				if(!isset($_POST['menu-item-laboratory-'.$key][$menu_item_db]))
				{
					$_POST['menu-item-laboratory-'.$key][$menu_item_db] = "";
				}
				
				$value = $_POST['menu-item-laboratory-'.$key][$menu_item_db];
				update_post_meta( $menu_item_db, '_menu-item-laboratory-'.$key, $value );
			}
		}
	}
}
...
if( !class_exists( 'Laboratory_Walker' ) )
{

	/**
	 * The laboratory walker is the frontend walker and necessary to display the menu, this is a advanced version of the wordpress menu walker
	 * @package WordPress
	 * @since 1.0.0
	 * @uses Walker
	 */
	class Laboratory_Walker extends Walker {
		/**
		...
		function end_lvl(&$output, $depth) {
			...
			if($depth === 0) 
			{
				if($this->mega_active)
				{
					...
					$output .= "\n</div>\n";
					$output = str_replace("{replace_one}", "<div class='laboratory_mega_div laboratory_mega".$this->max_columns."'>", $output);
					
					foreach($this->rowsCounter as $row => $columns)
					{
						$output = str_replace("{current_row_".$row."}", "laboratory_mega_menu_columns_".$columns, $output);
					}
		...
		function start_el(&$output, $item, $depth, $args) {
			...
			if($depth === 0)
			{	
				$this->mega_active = get_post_meta( $item->ID, '_menu-item-laboratory-megamenu', true);
			}
			...
			if($depth === 1 && $this->mega_active)
			{
				...
				if($this->columns > $args->max_columns || (get_post_meta( $item->ID, '_menu-item-laboratory-division', true) && $this->columns != 1))
				{
					$this->columns = 1;
					$this->rows ++;
					$output .= "\n<li class='laboratory_mega_hr'></li>\n";
				}
				...
				if($this->columns == 1)
				{
					$column_class  .= " laboratory_mega_menu_columns_first";
				}
			}
			else if($depth >= 2 && $this->mega_active && get_post_meta( $item->ID, '_menu-item-laboratory-textarea', true) )
			{
				$li_text_block_class = 'laboratory_mega_text_block ';
...
if( !class_exists( 'Laboratory_Backend_Walker' ) )
{

	class Laboratory_Backend_Walker extends Walker_Nav_Menu  
	{
		/**
		...
		function start_el(&$output, $item, $depth, $args) {
			...
			if($depth == 0)
			{
				$itemValue = get_post_meta( $item->ID, '_menu-item-laboratory-megamenu', true);
				if($itemValue != "") $itemValue = 'laboratory_mega_active ';
			}
			...
			<li id="menu-item-<?php echo $item_id; ?>" class="<?php echo $itemValue; echo implode(' ', $classes ); ?>">
				<dl class="menu-item-bar">
					<dt class="menu-item-handle">
						...
						<span class="item-controls">
							...
							<span class="item-type item-type-laboratory"><?php _e('Column'); ?></span>
				...
				<div class="menu-item-settings" id="menu-item-settings-<?php echo $item_id; ?>">
					...
					<?php endif; ?>
					<p class="description description-thin description-label laboratory_label_desc_on_active">
						<label for="edit-menu-item-title-<?php echo $item_id; ?>">
						<span class='laboratory_default_label'><?php _e( 'Navigation Label' ); ?></span>
						<span class='laboratory_mega_label'><?php _e( 'Mega Menu Column Title <span class="laboratory_supersmall">(if you dont want to display a title just enter a single dash: "-" )</span>' ); ?></span>
					...
					<div class='laboratory_mega_menu_options'>
					<!-- ################# laboratory custom code here ################# -->
						<?php
						$title = 'Use as Mega Menu';
						$key = "menu-item-laboratory-megamenu";
						$value = get_post_meta( $item->ID, '_'.$key, true);
						
						if($value != "") $value = "checked='checked'";
						?>
						
						<p class="description description-wide laboratory_checkbox laboratory_mega_menu laboratory_mega_menu_d0">
						...
						$key = "menu-item-laboratory-division";
						$value = get_post_meta( $item->ID, '_'.$key, true);
						
						if($value != "") $value = "checked='checked'";
						?>
						
						<p class="description description-wide laboratory_checkbox laboratory_mega_menu laboratory_mega_menu_d1">
							<label for="edit-<?php echo $key.'-'.$item_id; ?>">
								<input type="checkbox" value="active" id="edit-<?php echo $key.'-'.$item_id; ?>" class=" <?php echo $key; ?>" name="<?php echo $key . "[". $item_id ."]";?>" <?php echo $value; ?> /><?php _e( $title ); ?>
							</label>
						</p>
						...
						$key = "menu-item-laboratory-textarea";
						$value = get_post_meta( $item->ID, '_'.$key, true);
						
						if($value != "") $value = "checked='checked'";
						?>
						
						<p class="description description-wide laboratory_checkbox laboratory_mega_menu laboratory_mega_menu_d2">
							<label for="edit-<?php echo $key.'-'.$item_id; ?>">
								<input type="checkbox" value="active" id="edit-<?php echo $key.'-'.$item_id; ?>" class=" <?php echo $key; ?>" name="<?php echo $key . "[". $item_id ."]";?>" <?php echo $value; ?> /><span class='laboratory_long_desc'><?php _e( $title ); ?></span>
							</label>
						</p>
						<!-- ***************  end item *************** -->
					...
					<!-- ################# end laboratory custom code here ################# -->
