...
class COLABS7_ContactForm {

	const post_type = 'colabs7_contact_form';
	...
	public static function register_post_type() {
		register_post_type( self::post_type, array(
			'labels' => array(
				'name' => __( 'Contact Forms', 'colabs7' ),
				'singular_name' => __( 'Contact Form', 'colabs7' ) ),
			'rewrite' => false,
			'query_var' => false ) );
	}
	...
	public function __construct( $post = null ) {
		...
		do_action_ref_array( 'colabs7_contact_form', array( &$this ) );
	}
	...
	function get_properties() {
		...
		return apply_filters( 'colabs7_contact_form_properties', $properties, $this );
	}

	// Return true if this form is the same one as currently POSTed.
	function is_posted() {
		if ( ! isset( $_POST['_colabs7_unit_tag'] ) || empty( $_POST['_colabs7_unit_tag'] ) )
			return false;

		if ( $this->unit_tag == $_POST['_colabs7_unit_tag'] )
			return true;
	...
	function form_html() {
		...
		$form = '<div class="colabs7" id="' . $this->unit_tag . '">';

		$url = colabs7_get_request_uri();
		...
		$url = apply_filters( 'colabs7_form_action_url', $url );

		$class = 'colabs7-form';

		if ( $this->is_posted() ) {
			if ( empty( $colabs7->result['valid'] ) )
				$class .= ' invalid';
			elseif ( ! empty( $colabs7->result['spam'] ) )
				$class .= ' spam';
			elseif ( ! empty( $colabs7->result['mail_sent'] ) )
				$class .= ' sent';
			else
				$class .= ' failed';
		}

		$class = apply_filters( 'colabs7_form_class_attr', $class );

		$enctype = apply_filters( 'colabs7_form_enctype', '' );

		$novalidate = apply_filters( 'colabs7_form_novalidate',
			colabs7_support_html5() ? ' novalidate="novalidate"' : '' );
	...
	function form_hidden_fields() {
		$hidden_fields = array(
			'_colabs7' => $this->id,
			'_colabs7_version' => COLABS7_VERSION,
			'_colabs7_locale' => $this->locale,
			'_colabs7_unit_tag' => $this->unit_tag );

		if ( COLABS7_VERIFY_NONCE )
			$hidden_fields['_wpnonce'] = colabs7_create_nonce( $this->id );
	...
	function form_response_output() {
		...
		$class = 'colabs7-response-output';
		$content = '';

		if ( $this->is_posted() ) { // Post response output for non-AJAX

			if ( empty( $colabs7->result['valid'] ) )
				$class .= ' colabs7-validation-errors';
			elseif ( ! empty( $colabs7->result['spam'] ) )
				$class .= ' colabs7-spam-blocked';
			elseif ( ! empty( $colabs7->result['mail_sent'] ) )
				$class .= ' colabs7-mail-sent-ok';
			else
				$class .= ' colabs7-mail-sent-ng';

			if ( ! empty( $colabs7->result['message'] ) )
				$content = $colabs7->result['message'];

		} else {
			$class .= ' colabs7-display-none';
		}
		...
		return apply_filters( 'colabs7_form_response_output',
	...
	function validation_error( $name ) {
		...
		if ( ! isset( $colabs7->result['invalid_reasons'][$name] ) )
			return '';

		$ve = trim( $colabs7->result['invalid_reasons'][$name] );

		if ( empty( $ve ) )
			return '';

		$ve = '<span class="colabs7-not-valid-tip-no-ajax">' . esc_html( $ve ) . '</span>';
		return apply_filters( 'colabs7_validation_error', $ve, $name, $this );
	}
	...
	function form_do_shortcode() {
		...
		if ( COLABS7_AUTOP ) {
			$form = $colabs7_shortcode_manager->normalize_shortcode( $form );
			$form = colabs7_autop( $form );
		}

		$form = $colabs7_shortcode_manager->do_shortcode( $form );
		$this->scanned_form_tags = $colabs7_shortcode_manager->scanned_tags;
	...
	function form_scan_shortcode( $cond = null ) {
		...
		if ( ! empty( $this->scanned_form_tags ) ) {
			...
			$scanned = $colabs7_shortcode_manager->scan_shortcode( $this->form );
	...
	function form_elements() {
		return apply_filters( 'colabs7_form_elements', $this->form_do_shortcode() );
	}
	...
	function setup_posted_data() {
		...
		foreach ( $fes as $fe ) {
			...
			if ( COLABS7_USE_PIPE && is_a( $pipes, 'COLABS7_Pipes' ) && ! $pipes->zero() ) {
				if ( is_array( $value) ) {
					$new_value = array();
		...
		$this->posted_data = apply_filters( 'colabs7_posted_data', $posted_data );
	...
	function submit( $ajax = false ) {
		...
		if ( ! $validation['valid'] ) { // Validation error occured
			...
			do_action_ref_array( 'colabs7_mail_sent', array( &$this ) );

			if ( $ajax ) {
				$on_sent_ok = $this->additional_setting( 'on_sent_ok', false );

				if ( ! empty( $on_sent_ok ) )
					$result['scripts_on_sent_ok'] = array_map( 'colabs7_strip_quote', $on_sent_ok );
			} else {
				$this->clear_post();
			}
			...
			do_action_ref_array( 'colabs7_mail_failed', array( &$this ) );
		}
		...
		if ( $ajax ) {
			...
			if ( ! empty( $on_submit ) )
				$result['scripts_on_submit'] = array_map( 'colabs7_strip_quote', $on_submit );
		}
	...
	function validate() {
		...
		foreach ( $fes as $fe ) {
			$result = apply_filters( 'colabs7_validate_' . $fe['type'], $result, $fe );
		}

		$result = apply_filters( 'colabs7_validate', $result );
	...
	function accepted() {
		...
		return apply_filters( 'colabs7_acceptance', $accepted );
	}

	function spam() {
		$spam = false;

		if ( COLABS7_VERIFY_NONCE && ! $this->verify_nonce() )
			$spam = true;

		if ( $this->blacklist_check() )
			$spam = true;

		return apply_filters( 'colabs7_spam', $spam );
	}

	function verify_nonce() {
		return colabs7_verify_nonce( $_POST['_wpnonce'], $this->id );
	}

	function blacklist_check() {
		$target = colabs7_array_flatten( $this->posted_data );
		$target[] = $_SERVER['REMOTE_ADDR'];
		$target[] = $_SERVER['HTTP_USER_AGENT'];

		$target = implode( "\n", $target );

		return colabs7_blacklist_check( $target );
	}
	...
	function mail() {
		...
		do_action_ref_array( 'colabs7_before_send_mail', array( &$this ) );
		...
		if ( $result ) {
			...
			$additional_mail = apply_filters_ref_array( 'colabs7_additional_mail',
	...
	function compose_mail( $mail_template, $send = true ) {
		...
		$components = apply_filters_ref_array( 'colabs7_mail_components',
	...
	function mail_callback( $matches, $html = false ) {
		...
		if ( isset( $this->posted_data[$tagname] ) ) {
			...
			$replaced = colabs7_flat_join( $submitted );
			...
			$replaced = apply_filters( 'colabs7_mail_tag_replaced', $replaced,
				$submitted, $html );

			return stripslashes( $replaced );
		}

		$special = apply_filters( 'colabs7_special_mail_tags', '', $tagname, $html );
	...
	function message( $status ) {
		...
		return apply_filters( 'colabs7_display_message', $message, $status );
	}
	...
	function save() {
		...
		if ( $post_id ) {
			foreach ( $props as $prop => $value )
				update_post_meta( $post_id, '_' . $prop, colabs7_normalize_newline_deep( $value ) );
			...
			if ( $this->initial ) {
				$this->initial = false;
				$this->id = $post_id;
				do_action_ref_array( 'colabs7_after_create', array( &$this ) );
			} else {
				do_action_ref_array( 'colabs7_after_update', array( &$this ) );
			}

			do_action_ref_array( 'colabs7_after_save', array( &$this ) );
		}
	...
	function copy() {
		...
		$new = apply_filters_ref_array( 'colabs7_copy', array( &$new, &$this ) );
...
function colabs7_contact_form( $id ) {
	$contact_form = new COLABS7_ContactForm( $id );
...
function colabs7_get_contact_form_by_old_id( $old_id ) {
	global $wpdb;
	...
	if ( $new_id = $wpdb->get_var( $q ) )
		return colabs7_contact_form( $new_id );
}

function colabs7_get_contact_form_by_title( $title ) {
	$page = get_page_by_title( $title, OBJECT, COLABS7_ContactForm::post_type );

	if ( $page )
		return colabs7_contact_form( $page->ID );

	return null;
}

function colabs7_get_contact_form_default_pack( $args = '' ) {
	global $l10n;
	...
	if ( $locale && $locale != get_locale() ) {
		$mo_orig = $l10n['colabs7'];
		unset( $l10n['colabs7'] );

		if ( 'en_US' != $locale ) {
			$mofile = colabs7_plugin_path( 'languages/colabs7-' . $locale . '.mo' );
			if ( ! load_textdomain( 'colabs7', $mofile ) ) {
				$l10n['colabs7'] = $mo_orig;
				unset( $mo_orig );
			}
		}
	}
	...
	$contact_form->title = ( $title ? $title : __( 'Untitled', 'colabs7' ) );
	$contact_form->locale = ( $locale ? $locale : get_locale() );

	$props = $contact_form->get_properties();

	foreach ( $props as $prop => $value )
		$contact_form->{$prop} = colabs7_get_default_template( $prop );

	if ( isset( $mo_orig ) )
		$l10n['colabs7'] = $mo_orig;

	$contact_form = apply_filters_ref_array( 'colabs7_contact_form_default_pack',
		array( &$contact_form, $args ) );

	return $contact_form;
}

function colabs7_get_current_contact_form() {
	global $colabs7_contact_form;

	if ( ! is_a( $colabs7_contact_form, 'COLABS7_ContactForm' ) )
		return null;

	return $colabs7_contact_form;
}

function colabs7_is_posted() {
...
function colabs7_get_validation_error( $name ) {
	if ( ! $contact_form = colabs7_get_current_contact_form() )
		return '';
...
function colabs7_get_message( $status ) {
	if ( ! $contact_form = colabs7_get_current_contact_form() )
		return '';
...
function colabs7_scan_shortcode( $cond = null ) {
	if ( ! $contact_form = colabs7_get_current_contact_form() )
		return null;
...
function colabs7_form_controls_class( $type, $default = '' ) {
	$type = trim( $type );
	$default = array_filter( explode( ' ', $default ) );

	$classes = array_merge( array( 'colabs7-form-control' ), $default );

	$typebase = rtrim( $type, '*' );
	$required = ( '*' == substr( $type, -1 ) );

	$classes[] = 'colabs7-' . $typebase;

	if ( $required )
		$classes[] = 'colabs7-validates-as-required';
