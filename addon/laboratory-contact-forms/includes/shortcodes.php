...
class COLABS7_ShortcodeManager {
	...
	function do_shortcode_tag( $m ) {
		...
		if ( is_array( $attr ) ) {
			if ( is_array( $attr['options'] ) ) {
				if ( $this->shortcode_tags[$tag]['has_name'] && ! empty( $attr['options'] ) ) {
					...
					if ( ! colabs7_is_name( $scanned_tag['name'] ) )
			...
			if ( COLABS7_USE_PIPE ) {
				$pipes = new COLABS7_Pipes( $scanned_tag['raw_values'] );
		...
		$scanned_tag = apply_filters( 'colabs7_form_tag', $scanned_tag, $this->exec );
	...
	function shortcode_parse_atts( $text ) {
		...
		if ( preg_match( $pattern, $text, $match ) ) {
			...
			if ( ! empty( $match[2] ) ) {
				...
				$atts['values'] = colabs7_strip_quote_deep( $matched_values[0] );
			}
		} else {
			$atts = $text;
		}
...
function colabs7_add_shortcode( $tag, $func, $has_name = false ) {
	global $colabs7_shortcode_manager;

	if ( is_a( $colabs7_shortcode_manager, 'COLABS7_ShortcodeManager' ) )
		return $colabs7_shortcode_manager->add_shortcode( $tag, $func, $has_name );
}

function colabs7_remove_shortcode( $tag ) {
	global $colabs7_shortcode_manager;

	if ( is_a( $colabs7_shortcode_manager, 'COLABS7_ShortcodeManager' ) )
		return $colabs7_shortcode_manager->remove_shortcode( $tag );
}

function colabs7_do_shortcode( $content ) {
	global $colabs7_shortcode_manager;

	if ( is_a( $colabs7_shortcode_manager, 'COLABS7_ShortcodeManager' ) )
		return $colabs7_shortcode_manager->do_shortcode( $content );
}

function colabs7_get_shortcode_regex() {
	global $colabs7_shortcode_manager;

	if ( is_a( $colabs7_shortcode_manager, 'COLABS7_ShortcodeManager' ) )
		return $colabs7_shortcode_manager->get_shortcode_regex();
}

class COLABS7_Shortcode {
