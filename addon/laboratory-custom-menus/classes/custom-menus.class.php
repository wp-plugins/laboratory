<?php
/**
 * Laboratory Custom Menus Class
 *
 * All functionality pertaining to the Custom Menus feature.
 *
 * @package WordPress
...
class Laboratory_Custom_Menu {
	...
	function meta_box() {
		...
		foreach ($post_types as $post_type) {
			if( post_type_supports( $post_type, 'laboratory-custom-menus' ) ) {
				add_meta_box('dm_custom_locations', __('Custom Menus', 'laboratory'), array(&$this, 'metabox_markup'), $post_type, 'side', 'default');
			}
		}
	...
	function metabox_markup() {
		...
		if (count($menu_locations) == 0)
			echo '<p>' . __("Your theme doesn't seem to have any pre-defined menu locations. :(", 'laboratory') . '</p>';
	...
	function add_post_type_support() {
		add_post_type_support( 'post', 'laboratory-custom-menus' ) ;
		add_post_type_support( 'page', 'laboratory-custom-menus' ) ;
	}
