 ...
 * - login_header_title()
 */
class Laboratory_Login_Branding extends Laboratory_Settings_API {
	...
	public function load_settings_screen () {
		...
		$this->settings_screen = new Laboratory_LoginBranding_Settings();
		
		/* Setup login branding data */
		$this->settings_screen->token = 'laboratory-login-branding';
		if ( is_admin() ) {
			$this->settings_screen->name = __( 'Custom Login', 'laboratory' );
			$this->settings_screen->menu_label = __( 'Custom Login', 'laboratory' );
			$this->settings_screen->page_slug = 'laboratory-login-branding';
		}
