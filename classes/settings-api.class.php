...
/**
 * Laboratory Settings API Class
 *
 * A settings API (wrapping the WordPress Settings API) for use with Laboratory components.
 *
 * @package WordPress
...
class Laboratory_Settings_API {
	...
	public function __construct () {
		...
		$this->page_slug = 'lab-settings-api';
		$this->views_path = $laboratory->base->views_path;
	...
	public function init_sections () {
		...
		_e( 'Override init_sections() in your class.', 'laboratory' );
	} // End init_sections()
	...
	public function init_fields () {
		...
		_e( 'Override init_fields() in your class.', 'laboratory' );
	} // End init_fields()
	...
	public function register_settings_screen () {
		...
		add_action( 'admin_print_styles', array( $laboratory->admin, 'admin_styles' ) );
	...
	public function settings_screen () {
		require_once( $this->views_path . 'settings-api.php' );
	} // End settings_screen()
    ...
    //form field html testing
	public function form_field_html ( $args ) {
		$options = $this->get_settings();
	...
	public function form_field_colourpicker ( $args ) {
		...
		echo '<input type="button" id="select-' . esc_attr( $args['key'] ) . '" class="button" value="' . esc_attr__( 'Select Colour', 'laboratory' ) . '" />' . "\n";

		echo '<a href="#" class="pickcolor hide-if-no-js" id="' . esc_attr( $args['key'] ) . '-example"></a>';
		
		if ( isset( $args['data']['default'] ) && ( $args['data']['default'] != '' ) ) {
			echo '<br /><span id="default-' . esc_attr( $args['key'] ) . '" class="default-colour">' . __( 'Default colour:', 'laboratory' ) . ' <span class="colour">' . strtolower( $args['data']['default'] ) . '</span></span>' . "\n";
		}
	...
	protected function add_error ( $key, $data ) {
		if ( isset( $data['error_message'] ) ) {
			...
			$message = sprintf( __( '%s is a required field', 'laboratory' ), $data['name'] );
		}
	...
	protected function parse_errors () {
		if ( count ( $this->errors ) > 0 ) {
			...
			$message = sprintf( __( '%s settings updated', 'laboratory' ), $this->name );
	...
	public function enqueue_scripts () {
		...
		if ( $this->has_colourpicker ) {
			wp_enqueue_script( 'laboratory-settings-colourpickers', $laboratory->base->assets_url . 'js/colourpickers.js', array( 'farbtastic' ), '1.0.0' );
		}

		if ( $this->has_tabs ) {
			wp_enqueue_script( 'laboratory-settings-tabs-navigation', $laboratory->base->assets_url . 'js/tabs-navigation.js', array( 'jquery' ), '1.0.0' );
		}
	} // End enqueue_scripts()
	...
	public function enqueue_styles () {
		global $laboratory;
		if ( $this->has_colourpicker ) {
			wp_enqueue_style( 'laboratory-settings-colourpickers', $laboratory->base->assets_url . 'css/colourpickers.css', '', '1.0.0' );
			wp_enqueue_style( 'farbtastic' );
		}

		wp_enqueue_style( $laboratory->base->token . '-admin' );

		wp_enqueue_style( 'laboratory-settings-api', $laboratory->base->assets_url . 'css/settings.css', '', '1.0.0' );
	} // End enqueue_styles()
